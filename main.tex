...
\begin{abstract}
This work investigates how model compilation and quantization affect a small neural network intended for resource-constrained deployment. A LeNet-style convolutional neural network is trained on MNIST, exported to ONNX, and compiled using Apache TVM for CPU execution. The ONNX and TVM versions are compared in terms of accuracy, model size, graph structure, and inference performance. Additionally, INT8 quantization (dynamic and static) is applied to the ONNX model. The results show that TVM preserves the model output but does not make it smaller or faster in this setup, and that INT8 quantization reduces model size by approximately $3.5\times$ with no accuracy loss but also does not improve latency at batch size 1. The main finding is that both compilation and quantization change the representation of the model, but on a workload this small the per-inference overhead prevents latency improvement without further steps such as hardware-specific tuning or deployment on a target without native floating-point support.
\end{abstract}
...
However, quantization may introduce approximation errors, which can affect model accuracy. In this project, INT8 quantization is applied as a supplementary experiment to test whether reduced precision improves model size or latency beyond what compilation alone provides.
...
The ONNX model is used as the baseline representation, while the TVM-compiled model represents the optimized version. The primary comparison is in FP32 form. A supplementary INT8 quantization step (dynamic and static, using ONNX Runtime's quantization toolkit) is included to test whether reduced precision changes the size and latency picture. The comparison focuses on identifying differences in model structure and computational characteristics between the two representations.
...
As a preliminary test of whether quantization improves on the FP32 baseline, the ONNX model was quantized to INT8 using ONNX Runtime's built-in quantization toolkit. Two variants were produced: a dynamic-quantized model (weights quantized offline, activations quantized at runtime) and a statically-quantized model (both weights and activations quantized using 200 calibration samples from the MNIST training set). The static variant uses per-channel weight quantization with unsigned 8-bit activations and signed 8-bit weights, emitting dedicated integer operators (\texttt{QLinearConv}, \texttt{QGemm}) rather than float operators wrapped in quantize/dequantize nodes. Results are summarized in Table~\ref{tab:int8}.

\begin{table}[H]
\centering
\caption{INT8 quantization results compared to the FP32 baseline (ONNX Runtime, batch size 1, 200 timed runs after 10 warm-up runs). Size ratio is FP32 divided by INT8.}
\label{tab:int8}
\begin{tabular}{lccc}
\hline
Variant & File size (bytes) & Accuracy & Mean latency (ms) \\
\hline
FP32 baseline & 249{,}343 & 0.9852 & 0.031 \\
INT8 dynamic & 72{,}103 & 0.9855 & 0.069 \\
INT8 static & 70{,}369 & 0.9851 & 0.051 \\
\hline
\end{tabular}
\end{table}

Quantization reduces model size by approximately $3.5\times$ (from 244~KB to around 69--70~KB) with no meaningful accuracy loss (within $\pm 0.03$ percentage points of FP32). However, neither INT8 variant is faster than the FP32 baseline at batch size 1. Dynamic quantization is roughly $2.2\times$ slower and static roughly $1.6\times$ slower. The reasons for this are discussed in Section~5.
...
\subsection{Why INT8 quantization did not improve latency}

The INT8 results in Table~\ref{tab:int8} mirror the TVM result: a real transformation was applied, the model genuinely uses integer arithmetic (the static variant's graph contains \texttt{QLinearConv} and \texttt{QGemm} rather than float operators), and yet inference is slower than FP32 at batch size 1. The reason is similar to why TVM was slower: per-inference overhead dominates when the workload itself is microsecond-scale.

Specifically, INT8 introduces quantize and dequantize steps at the input and output of each layer. For a large model the cost of these steps is amortized over the cheaper integer matmul, but for LeNet at batch size 1 the integer arithmetic is only a few microseconds and the bookkeeping (scale multiplies, zero-point adjustments, rounding, clamping) takes longer than the time saved by replacing FP32 SIMD with INT8 SIMD. Dynamic quantization is worse because it also computes per-tensor activation scales at runtime on every call.

The size reduction, however, is clear and robust: weights shrink by roughly $3.5\times$ with essentially no accuracy cost. On a memory-constrained target where model size (not latency on a laptop) is the binding constraint, this is the relevant win. On hardware without a floating-point unit, INT8 would also become a latency win by default because the FP32 alternative is software-emulated.

\subsection{Implications for deployment}

The TinyML scenarios that motivate this kind of pipeline --- microcontrollers, low-bandwidth OTA updates, tight power envelopes --- usually need much more than a change of artifact form. The INT8 results confirm that quantization delivers on model size but, at this scale, does not provide a latency benefit on a general-purpose CPU. For latency improvement, additional steps are needed: hardware-specific tuning (AutoTVM, Ansor \cite{zheng2020ansor}) for the actual deployment target, larger batch sizes where the integer arithmetic dominates over per-call overhead, or a target where integer arithmetic is natively faster than floating-point.

The practical implication is that compilation and quantization are both necessary components of a deployment pipeline, but neither alone is sufficient to improve all metrics simultaneously on a small workload at batch size 1.
...
A supplementary INT8 quantization experiment confirmed that quantization delivers a clear size reduction (${\sim}3.5\times$ smaller) with no meaningful accuracy loss, but also does not improve CPU latency at batch size 1 --- the per-inference overhead of quantize/dequantize steps exceeds the savings from integer arithmetic on a workload this small.

The practical takeaway is that both compilation and quantization are best understood as structural transformations rather than automatic speedups when applied to a tiny model at batch size 1 on a general-purpose CPU. The size benefit of quantization is real and deployment-relevant (especially for memory-constrained targets and OTA updates), but latency improvement requires further steps: hardware-specific autotuning, larger batch sizes, or a target where integer arithmetic has a native advantage over floating-point.
